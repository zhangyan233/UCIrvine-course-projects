		...
		try {
			...
			RegisterStudentHandler objCommandEventHandler6 =
				new RegisterStudentHandler(
					...
					EventBus.EV_SHOW);
			/**
			 * add a new handler to check whether a lecture booked by over 3 students*
			 */
			OverStudentInCourseHandler overStudentInCourseHandler7=
					new OverStudentInCourseHandler(
							db,
							EventBus.EV_OVERBOOK,
							EventBus.EV_SHOW
					);

			/**
			 * add a new handler to check whether a course is conflict with other classes registered by the student, and inform the result to register handler*
			 */
			CourseConflictHandler courseConflictHandler=new CourseConflictHandler(
					db,
					EventBus.EV_CourseConflict,
					EventBus.EV_REGISTER_STUDENT
