...
public class SinkFilter extends FilterFramework
{
		...
		while (true)
		{
			try
			{
				...
				if ( id == 0 )
				{
					...
					String altitudeStr=String.format("%3.5f",altitude);
					//judge if altitude has been replaced: 1:yes 0:no
					if(wildJump==1){
						altitudeStr=altitudeStr+'*';
					}

					//transfer all data to string and put them in information
					String data=TimeStampFormat.format(TimeStamp.getTime())+","+String.format("%3.5f",velocity)+","+altitudeStr+","
	...
	public static void output(ArrayList<String> information){
		...
		try {
			bw=new BufferedWriter(new FileWriter("OutputB.csv"));
